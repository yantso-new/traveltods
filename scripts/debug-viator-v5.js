...
async function test(label, item) {
    ...
    try {
        ...
        if (res.status === 200) console.log("SUCCESS!", txt.substring(0, 200));
        else console.log("Error:", txt.substring(0, 500));
...
(async () => {
    await test("Plural { searchType: 'DESTINATIONS' }", { searchType: "DESTINATIONS" });
    await test("Plural { searchType: 'PRODUCTS' }", { searchType: "PRODUCTS" });
})();
